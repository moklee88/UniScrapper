...
for each_url in all_url:
    ...
    # Find container
    containers = soup.find('div', {'class', 'course--keyfacts module__keyfacts'}). \
        find_all('div', {'class', 'module__summary--item'})

    # Find part time and full time
    duration_tag = []
    for tag in containers:
        if "Duration" in tag.text:
            duration_tag = tag.find('div', {'class', 'module__summary--content'})

    if "full-time" in duration_tag.text:
        course_data['Full_Time'] = "Yes"
    if "part-time" in duration_tag.text:
        course_data['Part_Time'] = "Yes"

    # Find duration
    duration = duration_tag.text.replace(' ', '').strip()
    ...
    for tag in containers:
        if "Campuses" in tag.text:
            location_tag = tag.find('div', {'class', 'module__summary--content'})
